...
Tier 0 mounts at problem load: `get_problem`, `get_work`, `get_focus`, `get_stuck_signal`, `ask_question`, `place_flag`, and `award_check`. Spending a token is an explicit user action. Each spend decrements the three-token budget for the current problem and unlocks exactly the next tier: analogy, nudge, then concrete next step. Unlocked tools mount live and animate in the sidebar. No tool reveals a full solution.
